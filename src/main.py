...
task_combinations = [
    c for r in range(2, len(tasks) + 1) for c in combinations(tasks, r)
]
...
def create_task_combination(model, method, seed, tasks):
    ...
    for task in tasks:
        ...
        task_vectors.append(
            create_task_vector(pretrained_checkpoint, finetuned_checkpoint)
        )
        ...
        for methods in methods:
            for seed in seeds:
                ...
                for tasks in task_combinations:
                    ...
                    task_vectors.update(
                        create_task_combination(model, methods, seed, tasks)
                    )
                    ...
                    save_dir = f"saves_bts_merged/{methods}/{model}/{'_'.join(tasks)}_{seed}"
